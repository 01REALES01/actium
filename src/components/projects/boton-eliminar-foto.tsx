...
import { eliminarFotoAction } from "@/lib/actions/proyectos";

type Props = {
  fotoId: string;
  proyectoId: string;
  /** Presente cuando se borra desde el parte diario, para revalidar esa ruta. */
  fecha?: string;
  /** Se ejecuta tras un borrado exitoso (por ejemplo, para cerrar el visor). */
  onEliminada?: (fotoId: string) => void;
};

export function BotonEliminarFoto({ fotoId, proyectoId, fecha, onEliminada }: Props) {
  ...
  const handleEliminar = (e: React.MouseEvent) => {
    ...
    const confirmar = window.confirm(
      "¿Está seguro de que desea eliminar esta foto de forma permanente? Esta acción no se puede deshacer.",
    );
    if (!confirmar) return;
    ...
    startTransition(async () => {
      try {
        await eliminarFotoAction({ fotoId, proyectoId, fecha });
        onEliminada?.(fotoId);
        router.refresh();
      } catch (err) {
        setError(err instanceof Error ? err.message : "No fue posible eliminar la foto. Intenta de nuevo.");
      }
    });
  };
