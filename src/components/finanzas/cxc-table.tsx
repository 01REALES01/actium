...
import { RegistrarCobroCuotaDialog } from "@/components/finanzas/registrar-cobro-cuota-dialog";
...
import type { CxCConRelaciones } from "@/lib/data/cxc";
...
export function CxCTable({
  ...
  puedeEscribir = false,
  mensajeVacio = "Aún no hay cuentas por cobrar registradas.",
}: {
  cuentas: CxCConRelaciones[];
        ...
        return (
          <div key={c.id} className="rounded-actium border border-[--border-subtle] bg-[--bg-elevated]">
            <div className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div>
                <p className="font-sans text-sm font-semibold text-[--text-primary]">{c.cliente_nombre}</p>
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <p className="text-sm font-medium text-[--text-primary]">
                    {formatCOP(c.monto_cobrado)} / {formatCOP(c.monto_total)}
                  </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {c.cuotas.map((cuota) => {
                    ...
                    const saldoCuota = cuota.monto - cuota.monto_cobrado;
                    return (
                      <tr key={cuota.id} className="border-b border-[--border-subtle] last:border-b-0 hover:bg-[--bg-hover]">
                        ...
                        <td className="px-4 py-2 text-right text-[--text-secondary]">{formatCOP(cuota.monto_cobrado)}</td>
