...
import { crearEmpresaAction } from "@/lib/actions/admin";
...
export function CrearEmpresaModal() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const input = {
      ...
      ciudad: String(fd.get("ciudad") ?? "").trim() || undefined,
    ...
    startTransition(async () => {
      try {
        await crearEmpresaAction(input);
        setOpen(false);
        router.refresh();
      } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-lg rounded-actium border border-white/10 bg-[#1A1A1A] p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <h3 className="font-sans text-lg font-semibold text-white">Nueva empresa</h3>
              <button
                type="button"
                onClick={() => !isPending && setOpen(false)}
            ...
            <form onSubmit={onSubmit} className="mt-5 space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input name="nombre" required minLength={2} className={inputClass} placeholder="Razón social" />
                </div>
                <div>
                  <label className={labelClass}>NIT *</label>
                  <input name="nit" required className={inputClass} placeholder="900123456-7" />
                </div>
                <div>
                  <label className={labelClass}>Ciudad</label>
                  <input name="ciudad" className={inputClass} placeholder="Bogotá" />
                </div>
                <div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <input name="direccion" className={inputClass} placeholder="Calle / Cra ..." />
                </div>
                <div className="sm:col-span-2">
                  <label className={labelClass}>Correo</label>
                  <input name="email" type="email" className={inputClass} placeholder="contacto@empresa.com" />
                </div>
              </div>
